...
export class RoadService {
    ...
    const projectItem: ProjectItemDto[] = await Promise.all(
      projects.map(async (project) => {
        ...
        return {
          id: project.id,
          image: project.image,
          author_category: project.authCategory,
    ...
    const processedProjects: ProjectDto[] = await Promise.all(
      projects.map(async (project) => {
        ...
        return {
          id: project.id,
          image: project.image,
          authorCategory: project.authCategory,
          author: project.members,
          title: project.name,
          inform: project.introduction,
  ...
  async getProjectDetail(
    ...
    const project = await this.prismaService.findOneProjectByProjectId(id);
  ...
  async searchProject(
    ...
    const projects = await this.prismaService.findAllProjectsContainsKeyword(
      word,
      page,
    );
  ...
  async writeProject(writeDto: WriteRequestDto): Promise<string> {
    const { id, user, auth_category, video_link, ...object } = writeDto;
    ...
    const result = await this.prismaService.saveProject(
